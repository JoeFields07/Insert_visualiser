...
class TurningToolVisualizer:
    ...
    def _generate_insert_and_forces(self):
        ...
        # -------------------------------------------------------------
        # 1. INSERT BASE GEOMETRY (Nose tip at [0, 0, 0])
        # -------------------------------------------------------------
        if self.insert_type in ['C', 'D', 'V', 'E', 'M', 'S']:
            ...
            half_w = ic / (2.0 * np.cos(eps / 2.0))
            d_center = ic / (2.0 * np.sin(eps / 2.0))
            edge_length = np.sqrt(half_w ** 2 + d_center ** 2)
            
            ins_top = np.array([
                [0.0, 0.0, 0.0],                        # Nose Tip
                [-edge_length, 0.0, 0.0],               # Right Corner
                [-edge_length * np.cos(eps) - edge_length, 0.0, -edge_length * np.sin(eps)],  # Back Corner
    ...
    def _transform_mesh(self, points):
        # Lookup nose angle for active shape
        angle_map = {'C': 80.0, 'D': 55.0, 'S': 90.0, 'T': 60.0, 'V': 35.0}
        eps_deg = angle_map.get(self.insert_type, 80.0)
        half_eps_rad = np.radians(eps_deg / 2.0)
    ...
    def update(self, val):
        # 1. Capture user view parameters (elev, azim, roll) BEFORE clear
        ...
        # 5. Workpiece Surface Mesh
        r_stock = 25.0
        if self.wp_visible:
            r_turned = max(1.0, r_stock - self.ap)
            theta = np.linspace(0, 2*np.pi, 30)

            z_uncut = np.linspace(0, 30, 10)
        ...
        # 8. Friction force arrows
        #self.ax.scatter(, , , color='red', s=60, zorder=10, label='Normal Force')
